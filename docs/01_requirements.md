...
## 8. 全社バックオフィス拡張スコープ（Phase 0〜5）

経理・会計コア機能（領域A〜E）の完成後、全社のバックオフィス業務を一気通貫で統合管理するため、以下のPhase 0〜5拡張が実装・検証されました。詳細な業務要件・DB制約仕様・レビュー往復の記録は [docs/backoffice_expansion_plan.md](backoffice_expansion_plan.md) を参照してください。

### 8.1 拡張フェーズ一覧

| Phase | 領域名称 | 主要機能・要件概要 | 参照 |
|---|---|---|---|
| **Phase 0** | 土台確立 | 全10ロールのRBACマトリクス整備、監査ログ検索強化、添付ファイル基盤拡張 | 計画書 §2 |
| **Phase 1** | 総務・法務・契約管理 | 契約書ライフサイクル（起草〜締結〜終了）、PDF条項AI抽出、更新期限自動アラート、契約書全文検索（pg_trgm）、社内稟議・汎用申請ワークフロー | 計画書 §3 |
| **Phase 2** | 購買・調達管理 | 品目別購買申請・多段階承認、発注書管理、納品受領・仕入請求書・発注書の「3点照合（Three-Way Matching）」、買掛金（`vendor_bills`）自動連動、購買ダッシュボード | 計画書 §4 |
| **Phase 3** | 人事労務・給与内製化 | 従業員マスタ、Web勤怠打刻・月次労働時間集計、有効期間付き社会保険料率・所得税源泉徴収税額表マスタ、確定後WORM不変性を持つ給与計算エンジン、Web給与明細PDF出力、2026年分（令和8年分）年末調整計算エンジン | 計画書 §5 |
| **Phase 4** | 営業事務・商流管理 | 見積書ライフサイクル（自動採番、改訂履歴リンク、確定後WORM不変性、PDF生成）、案件（Deal）パイプライン管理（ステージ進捗、受注・失注終端ロック）、契約書↔案件↔見積書の契約更新リンク連携（対称的WORM保護と二重変換防止アドバイザリロック）、営業KPIダッシュボード | 計画書 §6 |
| **Phase 5** | 統合最適化 | 財務・購買・人事・営業の4領域を統合する横断エグゼクティブダッシュボード、各業務画面に連動して次の推奨アクションを提示するルールベースAIレコメンドエンジン、技術的負債解消・RBACドリフト自動検知 | 計画書 §7 |
