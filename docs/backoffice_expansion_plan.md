...
| P2-T4 | 購買ダッシュボード・レポート | テナント内の購買状況（申請中・承認済み・発注済み件数、サプライヤー別支出等）の可視化 | P2-T1, P2-T2, P2-T3 | ⚠️ SO判定REQUEST CHANGES（コミットa651c28、RLS/RBAC/集計ロジックは良好だが月次推移が未検証・EXPLAIN検証が実質何でもPASSするロジック。修正指示済み・再レビュー待ち） |
