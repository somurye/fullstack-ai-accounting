...
| Phase 3 | 人事労務 | 🔵 着手中 | 3/4完了（P3-T1〜T3）、P3-T4残り | P3-T3は6回（最多） |
| Phase 4 | 営業事務 | 未着手 | - | - |
| Phase 5 | 統合最適化 | 未着手 | - | - |

### 直近のアクション（要フォローアップ）

- **DEBT-020**: P3-T3のmainマージ時、Docker停止のためmain上での実DB E2E
  （`verify_schema.py --use-docker`）が未実施。次にDockerが使えるタイミングで最優先実施。
- Phase 3の残りはP3-T4（給与明細発行・年末調整）のみ。
...
DEBT-001〜DEBT-020のうち、解消済みは DEBT-003, 004, 005, 006（4件）。DEBT-019は
「DEBTというより設計上の恒久的な境界の記録」として区別している。残りは概ねLOW〜MEDIUMの
将来対応事項であり、現時点でPhase 3以降の着手を妨げるものはない（6節に一覧）。
...
| P3-T4 | 給与明細発行・年末調整 | 給与明細のPDF発行、年末調整の計算・書類生成 | P3-T3 | プロンプト発行済み・着手待ち（Phase 3最終タスク） |
