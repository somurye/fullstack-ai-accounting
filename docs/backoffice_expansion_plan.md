...
| Phase 4 | 営業事務 | 🔵 着手中 | 0/4 | P4-T1が初回でREQUEST CHANGES（保留） |
| Phase 5 | 統合最適化 | 未着手 | - | - |

### 直近のアクション

- P4-T1（見積書）の完了報告に対し、ChatGPT(SO)がREQUEST CHANGES（現時点・証跡未提示のため
  最終判定保留）と判定。WORMトリガーと`superseded_by`更新の矛盾（BLOCKER-01）を最優先に、
  受注転換時のinvoice自動起票の範囲確認・同時実行耐性・invoice側tenant境界・認証主体・
  WORM対象範囲の網羅性・PDFの日本語文字対応の計7点をまとめたフォローアップ指示プロンプト
  （P4-T1-FIX）を作成した。次はこれをGeminiに渡す。
...
| タスクID | タスク名 | 概要 | 依存 | ステータス |
|----------|----------|------|------|-----------|
| P4-T1 | 見積書（見積作成・確定・受注転換） | 見積の作成・明細管理、`draft`→`sent`→`accepted`/`rejected`/`expired`の状態遷移、確定後の不変性、受注への一方向変換 | P0-T1, P0-T4 | 🔴 SO判定REQUEST CHANGES（現時点。証跡未提示のため最終判定保留。WORMと`superseded_by`更新の矛盾等5点の確認待ち。P4-T1-FIX対応中） |
...
# 受け入れ基準（Definition of Done）
- [x] 見積を作成・編集（draft状態のみ）・確定送付（sent）・一覧表示できる
- [x] `sent`後の見積の金額・明細への直接UPDATEがDBレベルで拒否されることを実DBで確認する
- [x] 見積の改訂（新バージョン発行）が、旧バージョンを書き換えずに実行できることを確認する
- [x] 同一見積からの受注転換が一度しか成功しないことを実DBで確認する（2回目はエラー）
- [x] 他テナントの見積・見積明細が一切見えないことをRLSで確認
- [x] permissionを持たないロールでは操作できないことを確認
- [x] migrationがappend-only・fail-closedの原則（本計画書0.4節）に従っている
- [x] Phase 0で確立した実DB E2E検証基盤で、上記すべてを実PostgreSQL上で確認し、
      結果を報告に添付する
- [x] 完了報告に正確なコミットSHA（git rev-parse HEAD）・ブランチ名・main...ブランチの
      比較URLを明記する（本計画書0.4節ルール4に従う）
- [x] feature/p4-t1-quotations ブランチにコミット・pushし、比較URLを報告に含める
      （本計画書0.4節に従う）
