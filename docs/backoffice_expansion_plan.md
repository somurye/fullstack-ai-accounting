...
| P3-T3 | 給与計算エンジン | 勤怠実績・基本給・手当・控除から給与を計算し、AI提案パターンで人間確認を経て確定する（5.2節の原則②に対応） | P3-T1, P3-T2 | ⚠️ SO判定REQUEST CHANGES（コミット574239b、承認履歴の実在・権限保有・自己承認排除はDBで検証できているが、「権限保持者IDへのなりすまし」は未証明。これはapp_runtime生SQL実行者を完全に信頼しない前提での限界であり、境界を明示した上でAPI経路での防御を最終確認するよう指示済み） |
