...
| P1-T3 | 契約RBAC強制・AI提案ライフサイクル正式化 | ~~承認ワークフロー統合~~（P1-T1で先行実装済みのため統合済み）→ **スコープ変更**: (1) DEBT-005: contract permissionのAPI認可強制、(2) `ai_suggestions.target_type/target_id`のライフサイクル正式決定、(3) 状態遷移・SoDの最終確認 | P0-T1, P0-T4, P1-T1, P1-T2 | ✅ 実装完了・実DB E2E検証全PASS（コミット済み、SOレビュー待ち） |
...
| 2.8.0 | P1-T3実装完了。PermissionsGuardによるAPI認可強制(DEBT-005)、ai_suggestions target_type/source_suggestion_idライフサイクル正式化、自動承認ルール混在防止トリガー(DEBT-006)を実装。実DB E2E全64項目PASS確認 |
