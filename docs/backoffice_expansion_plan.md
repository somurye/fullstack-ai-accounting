...
# 受け入れ基準（Definition of Done）
- [x] ダッシュボードAPIが正しい集計結果を返す
- [x] 他テナントのデータが集計結果に一切混入しないことを実DB E2Eで確認する
      （2テナントにそれぞれ発注データを用意し、互いの集計に影響しないことを確認）
- [x] purchase_request.view権限がないユーザーはダッシュボードにアクセスできない
- [x] 大量データでの集計クエリのパフォーマンスに明らかな問題がないか簡易的に確認する
      （インデックスが必要な場合は追加する）
- [x] Phase 0で確立した実DB E2E検証基盤で、上記すべてを確認し結果を報告に添付する
- [x] feature/p2-t4-purchase-dashboard ブランチにコミット・pushし、比較URLを報告に含める
...
#### 【P2-T4 完了実績】
- ブランチ: `feature/p2-t4-purchase-dashboard`
- コミットSHA: `768744e4957aa680f70da649b596fe972b5710f7`
- 比較URL: `https://github.com/somurye/fullstack-ai-accounting/compare/main...feature/p2-t4-purchase-dashboard`
- 実DB E2E検証: 125/125 PASS（テナント完全分離・二重RBAC・ステータス集計・サプライヤーランキング・検収待ち集計）
- Backend単体テスト: 141/141 PASS (13 suites)
- Frontendビルド: `tsc -b && vite build` エラーゼロで完了
