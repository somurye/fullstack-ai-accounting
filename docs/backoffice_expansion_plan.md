...
本文書は3500行を超える規模になっており、詳細な経緯は8節の変更履歴・各Phaseの
...
| Phase 3 | 人事労務 | ✅ 完了 | 4/4 | P3-T3は6回、P3-T4は2回 |
| Phase 4 | 営業事務 | 未着手 | - | - |
| Phase 5 | 統合最適化 | 未着手 | - | - |

### 直近のアクション（要フォローアップ）

- **DEBT-020**: 解消済み。Docker復旧に伴いmain上およびP3-T4マージ後の実DB E2E（154/154全件PASS）を確認完了。
- **Phase 3（人事労務）完了**: 全4タスク（P3-T1: 従業員・勤怠、P3-T2: 保険料率・税率マスタ、P3-T3: 給与計算エンジン、P3-T4: 給与明細発行・年末調整）が完了。次はPhase 4（営業事務）の計画・着手へ。
...
「DEBTというより設計上の恒久的な境界の記録」、DEBT-021は「tax_year=2026簡略モデルへの意図的なスコープ限定」として区別している。残りは概ねLOW〜MEDIUMの
将来対応事項であり、現時点でPhase 4以降の着手を妨げるものはない（6節に一覧）。
...
| DEBT-020 | P3-T3-MERGE | P3-T3のmain反映時、開発環境のDocker Desktopが停止していたため、`verify_schema.py --use-docker`によるmain上でのクリーンDB実DB E2E（P3-T1〜P3-T3の給与計算・勤怠・料率マスタ全体を含む）が未実施だった事象。 | MEDIUM（実DB検証はこのプロジェクトの中核ルールであるため早期解消が望ましい） | Docker復旧後にmain上で実DB E2E（147/147 PASS）を実施し、さらにP3-T4マージ後にも実DB E2E（154/154 PASS、クリーンDB再現性検証PASS）を完了した。 | 🟢 解消済み（P3-T4-VERIFY-AND-MERGEにて完遂） |
...
| 7.8.0 | Docker復旧に伴いP3-T4-VERIFY-AND-MERGEを完遂。main上での実DB E2E実行（147/147 PASS）によりDEBT-020を完全に解消。P3-T4（給与明細発行・年末調整）において実DB E2E（001〜025全件、40テスト項目全件PASS）を確認し、mainへマージ完了（マージコミット6be18a3）。Phase 3（人事労務）の全4タスク（P3-T1〜T4）が完了 |
