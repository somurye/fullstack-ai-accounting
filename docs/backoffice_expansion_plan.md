...
| Phase 4 | 営業事務 | 🔵 着手中 | 2/4（P4-T1・P4-T2 PASS、マージ待ち） | P4-T1: 5回、P4-T2: 2回 |
| Phase 5 | 統合最適化 | 未着手 | - | - |

### 直近のアクション

- P4-T2-VERIFYの完了報告に対し、ChatGPT(SO)が**P4-T2を正式PASS**と判定。前回の
  CONDITIONAL PASSで求めた5点（非terminalステージ遷移の設計意図・`owner_user_id`の
  FK/tenant境界・認証actor実装・`closed_at`後発改変防止・git diffスコープ）すべてが
  実DB証跡で確認され、追加FIXは不要と明言。マージ指示プロンプトを作成し、mainへの
  マージと（P4-T1のマージコミットSHA未確認分も含めた）再検証を指示した。
- P4-T2のマージを前提に、**P4-T3（契約更新連携）**のタスク詳細・実装指示プロンプトを
  作成した。Phase 1の契約更新期限アラート（P1-T4）と、P4-T1見積・P4-T2案件を、人間の
  明示的操作（ボタン押下）を介して連携する設計とし、`contract_renewal_links`テーブルで
  紐付けを記録する。
- 次はユーザーが【マージ指示プロンプト P4-T2】をGeminiに渡し、P4-T1・P4-T2双方のマージ
  コミットSHAを確認したうえでマージと再検証を実行する。完了報告を受け取ったら、P4-T1・
  P4-T2を完全クローズに更新する。その後【指示プロンプト P4-T3】をGeminiに渡す。
...
| P4-T3 | 契約更新連携 | Phase 1の契約更新期限アラート（P1-T4）と営業案件・見積を連携し、更新期限が近い契約から更新提案の案件・見積を起票できるようにする | P4-T1, P4-T2 | 🔲 指示プロンプト作成済み・Gemini実装待ち |
...
1. 【マージ指示プロンプト P4-T2】をGeminiに渡す。P4-T1・P4-T2双方のマージコミットSHAを
   確認し、マージ後の実DB E2E再検証（P4-T1 57項目・P4-T2 51項目）を実行する。完了報告を
   受け取ったら、P4-T1・P4-T2を完全クローズに更新する。
2. マージ完了後、【指示プロンプト P4-T3】（契約更新連携）をGeminiに渡し、Phase 4の
   タスク3に着手する。
3. P4-T3完了後、実際のテーブル定義・API形状を踏まえてP4-T4（営業ダッシュボード・
   レポート）のタスク分解・実装指示プロンプトをClaudeが作成する（Phase 0/1/2/3と同じ
   方針）。
