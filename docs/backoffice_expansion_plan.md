...
| P3-T2 | 保険料率・税率マスタ管理 | 健康保険・厚生年金・雇用保険の料率、所得税源泉徴収税額表、住民税率を有効期間付きで管理する基盤（5.2節の原則①に対応） | P0-T1 | ⚠️ SO判定REQUEST CHANGES（DB/RLS/RBAC/EXCLUDE制約は良好だが、過去・適用済みマスタが通常UPDATEで書き換え可能で「追記型・過去データ上書き禁止」の運用原則に反する。修正指示済み・再レビュー待ち） |
