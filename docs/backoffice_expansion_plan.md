...
| タスクID | タスク名 | 概要 | 依存 | ステータス |
|----------|----------|------|------|-----------|
| P3-T1 | 従業員マスタ・勤怠管理 | 従業員情報、打刻（出勤・退勤・休憩）、労働時間集計（所定内・時間外・深夜・休日労働の区分） | P0-T1, P0-T4 | 実装完了・実DB E2E 131/131 PASS（SOレビュー待ち） |
...
# 受け入れ基準（Definition of Done）
- [x] 従業員を登録・編集・検索できる
- [x] 打刻（出勤・退勤・休憩）を記録し、労働時間が正しく区分（所定内/時間外/深夜/休日）される
- [x] 他テナントの従業員・勤怠データが一切見えないことをRLSで確認
- [x] permissionを持たないロールでは操作できないことを確認
- [x] 労働時間区分の境界値（例: ちょうど8時間、22時ちょうど）でのテストケースを含める
...
- [x] feature/p3-t1-employees-attendance ブランチにコミット・pushし、比較URLを報告に
      含める（本計画書0.4節に従う）
