...
6. **DB最終防御の限界と受容境界（API経路での本人性担保）**: アプリケーション全体が単一の共有DBロール（`app_runtime`）でDB操作を行うアーキテクチャであるため、DBトリガーが検証できるのは構造的業務ルール（テナント境界、自己承認禁止、権限保有等）であり、生SQL実行者の本人性検証はスコープ外（受容境界）とする。正規のAPI経路（Controller / Service）においては、`approver_id` 等の操作主体IDが常にサーバー側の認証済みセッションから強制導出され、クライアント入力による上書き・なりすましが構造的に不可能であることを保証する（計画書0.5節）。

---

## 7. Phase 0〜5拡張を通じて確立された設計原則

バックオフィス拡張ロードマップ（Phase 0〜5）の実装および厳格なレビュー往復（SO/Gemini）を通じて、アーキテクチャ全体にわたり以下の設計原則が確立・統一されました。

### 7.1 既存ドメインServiceへの委譲パターン（集約層のロジック重複排除）
- 横断KPIダッシュボード（`ExecutiveDashboardService`）やAIレコメンド（`RecommendationsService`）等の集約・統合機能は、各ドメイン（財務、購買、人事給与、営業商流）の集約ロジックや直接SQLを再実装してはならない。
- 必ず既存のドメインService（`ReportsService`, `PurchaseDashboardService`, `PayrollCalculationsService`, `SalesDashboardService` 等）が提供する公開メソッドに処理を委譲し、KPI定義や計算ロジックの一元性を担保する（P5-T1にて確立）。

### 7.2 不変性（WORM）の厳密なモデル分離
「不変（WORM）」と称される要件について、以下の2つの状態遷移モデルを明確に区別して設計・実装・文書化する（P4-T1, P5-T2にて確立）。
1. **完全WORM（絶対的不変）**:
   - 行作成時からいかなる列のUPDATEも物理DELETEも一切許可しない。
   - 例: `audit_logs`（監査ログ）、`attachments`（証憑）、確定済み料率マスタ（`rate_masters`の適用日到来後）、契約更新リンク（`contract_renewal_links`の各FK）。
2. **終端ロック型WORM（一度限りの状態遷移を許可するappend-only）**:
   - 起票中（`draft`）は柔軟な変更・削除を許可するが、確定・発行・終端状態（`posted`, `issued`, `won`/`lost`, `accepted`/`dismissed`）への遷移は1度限りとし、遷移後は重要列・ステータスの改変および物理DELETEをDBトリガーで完全にロックする。
   - 終端状態からの差し戻しや不正な状態ジャンプはDBトリガー（エラーコード `55000` / `23001` 等）でfail-closedに拒絶する。
   - 例: `quotations`（見積書）、`deals`（案件）、`recommendations`（AI提案）、`general_requests`（汎用申請）。

### 7.3 双方向リンクの対称的保護
- 2つの集約が相互リンク（1対1または親子関係）を形成する場合、片側だけの制約では逆側からの不整合や不正改ざんを防げない。
- 例: 見積書（`quotations.converted_invoice_id`）と売上請求書（`invoices.source_quotation_id`）の連携では、双方のテーブルにDBトリガーを配置し、「見積側での二重請求書変換の禁止」「請求書側での元見積IDの改ざん・クリア禁止」を対称的に保護する（P4-T1にて確立）。
- さらに、Webリクエストの同時打鍵レースによる二重変換を防止するため、アプリケーション層でのPostgreSQLアドバイザリロック（`pg_advisory_xact_lock`）による直列化を併用する。

### 7.4 実DB検証における `app_runtime` ロール・テナントコンテキスト経由の必須化
- PostgreSQLのスーパーユーザー（`postgres`）やテーブルオーナーはRLSをバイパスできるため、superuser接続下でテストが成功しても「RLSが効いている」ことの証明にはならない。
- RLSの完全テナント分離・権限遮断を検証するためには、実際の運用接続ロールである `app_runtime` でDBに接続し、`SET LOCAL app.current_tenant_id` を明示的に設定したトランザクション下でクエリを発行する実DB E2Eテストが必須である（P4-T4にて確立）。
- テナントコンテキスト未設定時は0件を返すfail-closed設計、および他テナントIDへのアクセス遮断を自動テストで常時証明する。
